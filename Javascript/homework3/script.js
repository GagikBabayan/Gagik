...
let dataEmployee = JSON.parse(employeeData);

// 1. Ստեղծել նոր զանգված հետևյալ մոդելով { name , position, salary };
const newArrayEmployee = (data) =>
  data.map((x) => ({
    name: x.name,
    position: x.hr.position,
    salary: x.hr.salary,
  }));
console.log(newArrayEmployee(dataEmployee));

// 2. Ստեղծել աշխատակիցների ցուցակի նոր զանգված որոնց աշխատավարձը >=150000;

const newSalary = (data) =>
  data.filter((x) => Number(x.hr.salary.replace(/[^\w]/g, "") >= 150000));

console.log(newSalary(dataEmployee));

// 3. Գտնել ամենից վաղ աշխատակցին; ( new Date() )

const oldEmployee = (data) =>
  data.sort(
    (a, b) =>
      new Date(b.hr.start_date).getTime() - new Date(a.hr.start_date).getTime()
  )[data.length - 1];

console.log(oldEmployee(dataEmployee));
...
const deleteEmployee = (data) => {
  ...
  setTimeout(function () {
    let allEmployee = data.map((x) => x.name);
    console.log(`Մեր բոլոր աշխատակիցները։ ${allEmployee}`);
    const randomEmployee =
      allEmployee[Math.floor(Math.random() * allEmployee.length)];
    console.log(`Ներող ապեր քեզ դուրս ենք հանում: ${randomEmployee}`);
    data = data.filter((x) => x.name !== randomEmployee);
    allEmployee = data.map((x) => x.name);
